<div class="flex gap-2">
    <a href="{{ route('blog.show', $blog) }}" target="_blank" class="badge badge-gray" style="text-decoration:none">View</a>
    <a href="{{ route('admin.blogs.edit', $blog) }}" class="badge badge-blue w-8 h-8 justify-center p-0" style="text-decoration:none" title="Edit blog post" aria-label="Edit blog post">
    ...
    <form action="{{ route('admin.blogs.destroy', $blog) }}" method="POST" class="delete-form" data-delete-message="This blog post will be permanently deleted.">
        @csrf @method('DELETE')
        <button type="submit" class="badge badge-red w-8 h-8 justify-center p-0 border-0 cursor-pointer" title="Delete blog post" aria-label="Delete blog post">
