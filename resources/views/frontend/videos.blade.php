    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="video-catalog-layout">
            <aside class="video-catalog-sidebar">
                ...
                <ul>
                    @foreach($categories as $category => $categoryVideos)
                        <li>
                            <a href="#category-{{ Str::slug($category) }}" class="video-cat-link {{ $category === $selectedCategory ? 'active' : '' }}">
            ...
            <div class="video-catalog-main">
                @foreach($categories as $category => $categoryVideos)
                    <section id="category-{{ Str::slug($category) }}" class="video-section">
                        <div class="video-section-head">
                            <h2>{{ $category }}</h2>
                            <a href="{{ route('videos.category', ['category' => Str::slug($category)]) }}" class="video-view-all">View All ({{ $categoryVideos->count() }})</a>
                        </div>

                        @if($categoryVideos->count())
                            @php $displayVideos = $categoryVideos->take(2); @endphp
